...
static int __chordQuantum;
static int __trillQuantum;

void ComAqwertianFingeringQwertyMapper_SetChordQuantum(int quantum) {
    __chordQuantum = quantum;
}

void ComAqwertianFingeringQwertyMapper_SetTrillQuantum(int quantum) {
    __trillQuantum = quantum;
}

int ComAqwertianFingeringQwertyMapper_ChordQuantum() {
    return __chordQuantum;
}

int ComAqwertianFingeringQwertyMapper_TrillQuantum() {
    return __trillQuantum;
}
...
- (id)initWithComAqwertianFingeringQwertyMapper_AlgorithmEnum:(ComAqwertianFingeringQwertyMapper_AlgorithmEnum *)algorithm
  withComAqwertianFingeringQwertyMapper_LevelOfDifficultyEnum:(ComAqwertianFingeringQwertyMapper_LevelOfDifficultyEnum *)level {
   ...
   // rand.seed = 12523524;
  return [self initWithComAqwertianFingeringQwertyMapper_AlgorithmEnum:algorithm withComAqwertianFingeringQwertyMapper_LevelOfDifficultyEnum:level withJavaUtilRandom:rand];
}
...
//  if ([self isImpossibleFingerWithNSString:prevFinger withNSString:nextFinger]) return YES;
    ...
    BOOL valid = chordIsValid(prevNote.qwerty, key, prevNote.time, currNote.time, ComAqwertianFingeringQwertyMapper_ChordQuantum());
    if (!valid) {
        return YES;
    }
