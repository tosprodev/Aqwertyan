...
//
//  Created by nziebart on 2/21/13.
//
...
static IOSObjectArray * ComAqwertianFingeringUsageAlgorithm_CHORDING_EXPERT_;
static IOSObjectArray * ComAqwertianFingeringUsageAlgorithm_CHORDING_BEGINNER_;
static NSArray * TwoFinderChords;
...
- (void)set_chording:(IOSObjectArray *)new_chording {
  _chording_ = new_chording;
}
...
withComAqwertianFingeringQwertyMapper_LevelOfDifficultyEnum:(ComAqwertianFingeringQwertyMapper_LevelOfDifficultyEnum *)level {
  if ((self = [super init])) {
    ...
    switch ([level ordinal]) {
      ...
      _chording_ = ComAqwertianFingeringUsageAlgorithm_CHORDING_BEGINNER_;
      break;
      ...
      _chording_ = ComAqwertianFingeringUsageAlgorithm_CHORDING_EXPERT_;
      break;
      default:
      _frequency_ = ComAqwertianFingeringUsageAlgorithm_FREQUENCY_EXPERT_;
      _chording_ = ComAqwertianFingeringUsageAlgorithm_CHORDING_EXPERT_;
...
withComAqwertianFingeringPatternList:(ComAqwertianFingeringPatternList *)patterns
    ...
    for (int count = 0; count < (int) [((IOSObjectArray *) NIL_CHK(notes)) count]; count++) {
        ...
        skip = [self matchTrillWithNotes:notes index:count history:history];
        if (skip > 0) {
            count += skip - 1;
            continue;
        }
...
- (int) matchTrillWithNotes:(IOSObjectArray *)notes index:(int)index history:(ComAqwertianFingeringQwertyMapper_KeyHistory *)history {
    if (index + 2 >= notes.count) return 0;
    
    RGNote *firstNote =(RGNote *)[notes objectAtIndex:index];
    RGNote *secondNote = (RGNote *)[notes objectAtIndex:index+1];
    int evenNote = [firstNote note];
    int oddNote = [secondNote note];
    
    if (secondNote.time - firstNote.time > ComAqwertianFingeringQwertyMapper_TrillQuantum()) {
        return 0;
    }
    
    RGNote *thirdNote = (RGNote *)[notes objectAtIndex:index+2];
    
    if (thirdNote.time - secondNote.time > ComAqwertianFingeringQwertyMapper_TrillQuantum()) return 0;
    
    if (thirdNote.note == evenNote) {
        // we have a trill
        
        char trillKeys[2];
        if (([history isFingerDownWithNSString:@"J"]
             || [history isFingerDownWithNSString:@"K"]
             || arc4random()%2 == 0)
            &&
            !([history isFingerDownWithNSString:@"D"]
              || [history isFingerDownWithNSString:@"F"])){
            trillKeys[0] = 'D';
            trillKeys[1] = 'F';
        } else if (![history isFingerDownWithNSString:@"J"] && ![history isFingerDownWithNSString:@"K"]) {
            trillKeys[0] = 'J';
            trillKeys[1] = 'K';
        } else {
            return 0;
        ...
        int totalTrillNotes = 0;
        RGNote *previousNote;
        for (int i = index; i < notes.count; i++) {
            RGNote *n = [notes objectAtIndex:i];
            
            // Ensure that the note matches the trill ABAB pattern, and is close enough to the previous note in the trill
            if (i%2 == 0) {
                if (n.note != evenNote) break;
            } else {
                if (n.note != oddNote) break;
            }
            if (previousNote && n.time - previousNote.time > ComAqwertianFingeringQwertyMapper_TrillQuantum()) break;
            
            n.qwerty = trillKeys[i%2];
            n.isTrill = YES;
            totalTrillNotes++;
        }
...
- (int)matchChordWithComAqwertianFingeringMusicFile_NoteArray:(IOSObjectArray *)notes
  ...
  for (int i = count + 1; i < (int) [((IOSObjectArray *) NIL_CHK(notes)) count]; i++) {
    ComAqwertianFingeringMusicFile_Note *n = [((IOSObjectArray *) NIL_CHK(notes)) objectAtIndex:i];
      ...
      // Instead of looking for equal times, check if they are close enough together that they can be counted as a chord
      if (ABS(prev.time - n.time) <= ComAqwertianFingeringQwertyMapper_ChordQuantum()) {
         [((id<JavaUtilList>) NIL_CHK(chord)) addWithId:n];
          prev = n;
      }
  }
  if ([((id<JavaUtilList>) NIL_CHK(chord)) size] >= 2 && [((id<JavaUtilList>) NIL_CHK(chord)) size] <= 7) skip = [self processChordWithJavaUtilList:chord];
  return skip;
}

+ (IOSObjectArray *)CHORDING_EXPERT {
  return ComAqwertianFingeringUsageAlgorithm_CHORDING_EXPERT_;
}

+ (void)setCHORDING_EXPERTWithComAqwertianFingeringUsageAlgorithm_ChordFingerArray:(IOSObjectArray *)CHORDING_EXPERT {
  ComAqwertianFingeringUsageAlgorithm_CHORDING_EXPERT_ = CHORDING_EXPERT;
}

+ (IOSObjectArray *)CHORDING_BEGINNER {
  return ComAqwertianFingeringUsageAlgorithm_CHORDING_BEGINNER_;
}

+ (void)setCHORDING_BEGINNERWithComAqwertianFingeringUsageAlgorithm_ChordFingerArray:(IOSObjectArray *)CHORDING_BEGINNER {
  ComAqwertianFingeringUsageAlgorithm_CHORDING_BEGINNER_ = CHORDING_BEGINNER;
}
...
- (int)processChordWithJavaUtilList:(id<JavaUtilList>)chord {
  ...
  ComAqwertianFingeringUsageAlgorithm_ChordFinger *finger = [self findChordFingeringWithInt:((ComAqwertianFingeringMusicFile_Note *) NIL_CHK(start)).note count:[chord size]];
...
- (ComAqwertianFingeringUsageAlgorithm_ChordFinger *)findChordFingeringWithInt:(int)note count:(int)count {
    if (count == 2) {
        for (int i = 0; i < (int) [TwoFinderChords count]; i++) {
            if (note <= ((ComAqwertianFingeringUsageAlgorithm_ChordFinger *) [TwoFinderChords objectAtIndex:i]).maxMidi) return [TwoFinderChords objectAtIndex:i];
        }
    } else if (count > 4) {
        if (note < 60) return [_chording_ objectAtIndex:1];
        return [_chording_ objectAtIndex:4];
    } else {
        for (int i = 0; i < (int) [((IOSObjectArray *) NIL_CHK(_chording_)) count]; i++) {
            if (note <= ((ComAqwertianFingeringUsageAlgorithm_ChordFinger *) [((IOSObjectArray *) NIL_CHK(_chording_)) objectAtIndex:i]).maxMidi) return [((IOSObjectArray *) NIL_CHK(_chording_)) objectAtIndex:i];
        }
    }
    return nil;
}
...
+ (void)initialize {
  if (self == [ComAqwertianFingeringUsageAlgorithm class]) {
    ...
    ComAqwertianFingeringUsageAlgorithm_CHORDING_EXPERT_ = [IOSObjectArray arrayWithObjects:(id[]){ [[ComAqwertianFingeringUsageAlgorithm_ChordFinger alloc] initWithInt:19 withNSString:@"VCXZ"], [[ComAqwertianFingeringUsageAlgorithm_ChordFinger alloc] initWithInt:31 withNSString:@"FDSA"], [[ComAqwertianFingeringUsageAlgorithm_ChordFinger alloc] initWithInt:43 withNSString:@"REWQ"], [[ComAqwertianFingeringUsageAlgorithm_ChordFinger alloc] initWithInt:55 withNSString:@"4321"], [[ComAqwertianFingeringUsageAlgorithm_ChordFinger alloc] initWithInt:67 withNSString:@"M,./"], [[ComAqwertianFingeringUsageAlgorithm_ChordFinger alloc] initWithInt:79 withNSString:@"JKL;"], [[ComAqwertianFingeringUsageAlgorithm_ChordFinger alloc] initWithInt:91 withNSString:@"UIOP"], [[ComAqwertianFingeringUsageAlgorithm_ChordFinger alloc] initWithInt:999 withNSString:@"7890"] } count:8 type:[IOSClass classWithClass:[ComAqwertianFingeringUsageAlgorithm_ChordFinger class]]];
      
    ComAqwertianFingeringUsageAlgorithm_CHORDING_BEGINNER_ = [IOSObjectArray arrayWithObjects:(id[]){ [[ComAqwertianFingeringUsageAlgorithm_ChordFinger alloc] initWithInt:20 withNSString:@"ZXCV/.,M"], [[ComAqwertianFingeringUsageAlgorithm_ChordFinger alloc] initWithInt:35 withNSString:@"ASDF;LKJ"], [[ComAqwertianFingeringUsageAlgorithm_ChordFinger alloc] initWithInt:50 withNSString:@"QWERPOIU"], [[ComAqwertianFingeringUsageAlgorithm_ChordFinger alloc] initWithInt:75 withNSString:@"/.,MZXCV"], [[ComAqwertianFingeringUsageAlgorithm_ChordFinger alloc] initWithInt:90 withNSString:@";LKJASDF"], [[ComAqwertianFingeringUsageAlgorithm_ChordFinger alloc] initWithInt:999 withNSString:@"POIUQWER"] } count:6 type:[IOSClass classWithClass:[ComAqwertianFingeringUsageAlgorithm_ChordFinger class]]];
      
      NSArray *chords = @[@"XC",@"XV",@"CV",@"SD",@"SF",@"DF", @"WE",@"WR",@"ER",@"LK",@"LJ",@"KJ",@"KH",@"UI",@"OI",@"OU"];
      NSMutableArray *fingers = @[].mutableCopy;
     
      int note = 21;
      int increment = 5;
      
      for (NSString *chord in chords) {
          ComAqwertianFingeringUsageAlgorithm_ChordFinger *finger = [[ComAqwertianFingeringUsageAlgorithm_ChordFinger alloc] initWithInt:note withNSString:chord];
          [fingers addObject:finger];
          note += increment;
      }
      ComAqwertianFingeringUsageAlgorithm_ChordFinger *last = fingers.lastObject;
      [last setMaxMidi:999];
      
      TwoFinderChords = fingers;
...
- (id)initWithInt:(int)max
  ...
  if ((self = [super init])) {
    maxMidi_ = max;
    fingers_ = [[IOSCharArray alloc] initWithLength:f.length];
      
      for (int i = 0; i < f.length; i++) {
    [((IOSCharArray *) NIL_CHK(fingers_)) replaceCharAtIndex:i withChar:[NIL_CHK(f) charAtWithInt:i]];
      }
