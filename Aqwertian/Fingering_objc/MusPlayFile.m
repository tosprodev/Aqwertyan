...
//  source: /Users/nziebart/GoogleDrive/Aqwertian/MusPlayFile.java
//
...
- (void)set_repeats:(id<JavaUtilList>)new_repeats {
  [_repeats_ autorelease];
  _repeats_ = [new_repeats retain];
}
  ...
  if ((self = [super init])) {
    ...
    while ((line = [((JavaIoBufferedReader *) NIL_CHK(bin)) readLine]) != nil) {
      ...
      if ([((AqwertianFingeringUtilStringTokenizer *) NIL_CHK(t)) hasMoreTokens]) {
        if (note == nil) note = [[[AqwertianFingeringMusPlayFile_MusPlayNote alloc] initWithAqwertianFingeringMusPlayFile_MusPlayNote:previous] autorelease];
        NSString *cmd = [((AqwertianFingeringUtilStringTokenizer *) NIL_CHK(t)) nextToken];
        BOOL isNote = NO;
        if ([@"NAME" isEqual:cmd]) ([_name_ autorelease], _name_ = [[NIL_CHK(line) substring:[@"NAME " length]] retain]);
          ...
          AqwertianFingeringMusPlayFile_MusPlayNote *tmp = [[[AqwertianFingeringMusPlayFile_MusPlayNote alloc] init] autorelease];
          ...
          AqwertianFingeringMusPlayFile_Repeat *r = [[[AqwertianFingeringMusPlayFile_Repeat alloc] init] autorelease];
...
- (void)processRepeats {
  id<JavaUtilList> newNotes = [[[JavaUtilArrayList alloc] init] autorelease];
  {
...
- (void)storeWithJavaIoOutputStream:(JavaIoOutputStream *)o {
  ...
  AqwertianFingeringMusPlayFile_MusPlayNote *prev = [[[AqwertianFingeringMusPlayFile_MusPlayNote alloc] init] autorelease];
...
+ (void)initialize {
  if (self == [AqwertianFingeringMusPlayFile class]) {
    AqwertianFingeringMusPlayFile_TONE_ = [[IOSObjectArray arrayWithObjects:(id[]){ @"C", @"C#", @"D", @"D#", @"E", @"F", @"F#", @"G", @"G#", @"A", @"A#", @"B" } count:12 type:[IOSClass classWithClass:[NSString class]]] retain];
  }
}

- (void)dealloc {
  ([_repeats_ autorelease], _repeats_ = nil);
  ([_events_ autorelease], _events_ = nil);
  ([_name_ autorelease], _name_ = nil);
  [super dealloc];
}
...
- (id)init {
  if ((self = [super initWithInt:0 withInt:0 withInt:0 withInt:0])) {
    ...
    ([key_ autorelease], key_ = [@"" retain]);
    ([tempo_ autorelease], tempo_ = [@"" retain]);
    measure_ = 1;
    ([line_ autorelease], line_ = [@"" retain]);
    volume_ = 100;
...
- (id)initWithAqwertianFingeringMusPlayFile_MusPlayNote:(AqwertianFingeringMusPlayFile_MusPlayNote *)previous {
  if ((self = [self init])) {
    if (previous != nil) {
      ([meter_ autorelease], meter_ = [(AqwertianFingeringMusPlayFile_MusPlayNote_Duration *) [((AqwertianFingeringMusPlayFile_MusPlayNote_Duration *) NIL_CHK(previous.meter)) clone] retain]);
      ([key_ autorelease], key_ = [previous.key retain]);
      ([tempo_ autorelease], tempo_ = [previous.tempo retain]);
      measure_ = previous.measure;
      ([line_ autorelease], line_ = [previous.line retain]);
...
- (void)dealloc {
  ([length_ autorelease], length_ = nil);
  ([line_ autorelease], line_ = nil);
  ([tempo_ autorelease], tempo_ = nil);
  ([key_ autorelease], key_ = nil);
  ([meter_ autorelease], meter_ = nil);
  [super dealloc];
}
