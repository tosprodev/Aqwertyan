...
//  source: /Users/nziebart/GoogleDrive/Aqwertian/MusicFile.java
//
...
@implementation AqwertianFingeringMusicFile
...
- (void)sortNotesWithJavaUtilList:(id<JavaUtilList>)notes {
  ...
  AqwertianFingeringMusicFile_Note *prev = nil;
  for (id<JavaUtilIterator> it = ((id<JavaUtilIterator>) [((id<JavaUtilList>) NIL_CHK(notes)) iterator]); [((id<JavaUtilIterator>) NIL_CHK(it)) hasNext]; ) {
    ...
    if ([self sameNoteWithAqwertianFingeringMusicFile_Note:prev withAqwertianFingeringMusicFile_Note:n]) {
...
- (BOOL)sameNoteWithAqwertianFingeringMusicFile_Note:(AqwertianFingeringMusicFile_Note *)n1
  ...
  return ((AqwertianFingeringMusicFile_Note *) NIL_CHK(n1)).note == ((AqwertianFingeringMusicFile_Note *) NIL_CHK(n2)).note && ((AqwertianFingeringMusicFile_Note *) NIL_CHK(n1)).duration == ((AqwertianFingeringMusicFile_Note *) NIL_CHK(n2)).duration && ((AqwertianFingeringMusicFile_Note *) NIL_CHK(n1)).time == ((AqwertianFingeringMusicFile_Note *) NIL_CHK(n2)).time;
}
...
@implementation AqwertianFingeringMusicFile_Note

static IOSObjectArray * AqwertianFingeringMusicFile_Note_NOTES_;
...
+ (IOSObjectArray *)NOTES {
  return AqwertianFingeringMusicFile_Note_NOTES_;
}

- (NSString *)getNoteName {
  return [AqwertianFingeringMusicFile_Note getNoteNameWithInt:note_];
}

+ (NSString *)getNoteNameWithInt:(int)note {
  return [((IOSObjectArray *) NIL_CHK(AqwertianFingeringMusicFile_Note_NOTES_)) objectAtIndex:note % 12];
}

- (int)getOctave {
  return [AqwertianFingeringMusicFile_Note getOctaveWithInt:note_];
}
...
- (NSString *)getNoteValue {
  return [AqwertianFingeringMusicFile_Note getNoteValueWithInt:note_];
}

+ (NSString *)getNoteValueWithInt:(int)note {
  return [NSString stringWithFormat:@"%d%@", [AqwertianFingeringMusicFile_Note getOctaveWithInt:note], [AqwertianFingeringMusicFile_Note getNoteNameWithInt:note]];
}

- (int)compareToWithId:(AqwertianFingeringMusicFile_Note *)n {
  int ret = time_ - ((AqwertianFingeringMusicFile_Note *) NIL_CHK(n)).time;
  if (ret != 0) return ret;
  ret = duration_ - ((AqwertianFingeringMusicFile_Note *) NIL_CHK(n)).duration;
  if (ret != 0) return ret;
  return note_ - ((AqwertianFingeringMusicFile_Note *) NIL_CHK(n)).note;
}
...
+ (void)initialize {
  if (self == [AqwertianFingeringMusicFile_Note class]) {
    AqwertianFingeringMusicFile_Note_NOTES_ = [[IOSObjectArray arrayWithObjects:(id[]){ @"C ", @"C#", @"D ", @"D#", @"E ", @"F ", @"F#", @"G ", @"G#", @"A ", @"A#", @"B " } count:12 type:[IOSClass classWithClass:[NSString class]]] retain];
  }
}
