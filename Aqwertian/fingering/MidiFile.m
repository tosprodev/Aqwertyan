    ...
    if ([((AqwertianFingeringMidiFile_Event *) NIL_CHK(e)) isNoteOn]) {
      ...
      for (int j = i + 1; j < [((id<JavaUtilList>) NIL_CHK(_events_)) size]; j++) {
        ...
        if (!((AqwertianFingeringMidiFile_Event *) NIL_CHK(f)).visited && [((AqwertianFingeringMidiFile_Event *) NIL_CHK(f)) isNoteOff] && channel == [((AqwertianFingeringMidiFile_Event *) NIL_CHK(f)) getChannel] && note == [((AqwertianFingeringMidiFile_Event *) NIL_CHK(f)) getNote]) {
          ...
          foundNoteOff = YES;
          [((id<JavaUtilList>) NIL_CHK(notes)) addWithId:[[AqwertianFingeringMusicFile_Note alloc] initWithInt:channel withInt:((AqwertianFingeringMidiFile_Event *) NIL_CHK(e)).time withInt:((AqwertianFingeringMidiFile_Event *) NIL_CHK(f)).time - ((AqwertianFingeringMidiFile_Event *) NIL_CHK(e)).time withInt:note]];
          break;
        }
      }
      if (!foundNoteOff) [((id<JavaUtilList>) NIL_CHK(notes)) addWithId:[[AqwertianFingeringMusicFile_Note alloc] initWithInt:channel withInt:((AqwertianFingeringMidiFile_Event *) NIL_CHK(e)).time withInt:[self getDefaultDuration] withInt:note]];
    }
  }
  for (int i = 0; i < [((id<JavaUtilList>) NIL_CHK(_events_)) size]; i++) {
    AqwertianFingeringMidiFile_Event *e = ((AqwertianFingeringMidiFile_Event *) [((id<JavaUtilList>) NIL_CHK(_events_)) getWithInt:i]);
...
- (AqwertianFingeringMidiFile_Chunk *)readChunkWithJavaIoInputStream:(JavaIoInputStream *)inArg {
  ...
  if ([@"MThd" isEqual:id_]) return [[AqwertianFingeringMidiFile_MThdChunk alloc] initWithNSString:id_ withInt:length withJavaLangByteArray:data];
  if ([@"MTrk" isEqual:id_]) return [[AqwertianFingeringMidiFile_MTrkChunk alloc] initWithNSString:id_ withInt:length withJavaLangByteArray:data];
  return [[AqwertianFingeringMidiFile_GenericChunk alloc] initWithNSString:id_ withInt:length];
}
...
+ (AqwertianFingeringMidiFile_LengthInfo *)readVarLengthWithJavaLangByteArray:(IOSByteArray *)data
  ...
  return [[AqwertianFingeringMidiFile_LengthInfo alloc] initWithInt:len withInt:offset];
}
...
- (void)parseChunkWithInt:(int)len
  ...
  while (offset < len) {
    ...
    AqwertianFingeringMidiFile_Event *e = [[AqwertianFingeringMidiFile_Event alloc] init];
