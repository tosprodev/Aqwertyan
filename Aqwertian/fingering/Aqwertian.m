...
+ (void)printStatsWithAqwertianFingeringQwertyMapper:(AqwertianFingeringQwertyMapper *)qm
  ...
  NSLog(@"%@", [((AqwertianFingeringQwertyMapper *) NIL_CHK(qm)) getInfo]);
  {
    ...
    while ([iter__ hasNext]) {
      ...
      if (--count >= 0) NSLog(@"%@", [((AqwertianFingeringQwertyMapper *) NIL_CHK(qm)) toStringWithAqwertianFingeringMusicFile_Note:note]);
      else break;
    }
  }
  NSLog(@"");
...
+ (AqwertianFingeringMusicFile *)createMusicFileWithNSString:(NSString *)fileName {
  ...
  if ([NIL_CHK(fileName) hasSuffix:@".mus"]) mf = [[AqwertianFingeringMusPlayFile alloc] initWithJavaIoInputStream:file];
  else if ([NIL_CHK(fileName) hasSuffix:@".nts"]) mf = [[AqwertianFingeringMusNotesFile alloc] initWithJavaIoInputStream:file];
  else mf = [[AqwertianFingeringMidiFile alloc] initWithJavaIoInputStream:file];
...
+ (AqwertianFingeringPatternList *)getPatternsWithNSString:(NSString *)fileName {
  @try {
    ...
    AqwertianFingeringPatternList *patterns = [[AqwertianFingeringPatternList alloc] initWithJavaIoReader:pfile];
    [((JavaIoFileReader *) NIL_CHK(pfile)) close];
    return patterns;
  }
  @catch (JavaIoIOException *e) {
    return [[AqwertianFingeringPatternList alloc] init];
  }
}
...
+ (void)aqw_mainWithNSStringArray:(IOSObjectArray *)args {
  @try {
    ...
    id<JavaUtilCollection> notes = [((AqwertianFingeringMusicFile *) NIL_CHK(midi)) getNotesWithJavaUtilCollection:[[JavaUtilArrayList alloc] init]];
    AqwertianFingeringQwertyMapper *qm = [[AqwertianFingeringQwertyMapper alloc] initWithAqwertianFingeringQwertyMapper_AlgorithmEnum:AqwertianFingeringAqwertian_ALGO_ withAqwertianFingeringQwertyMapper_LevelOfDifficultyEnum:level];
...
+ (void)initialize {
  if (self == [AqwertianFingeringAqwertian class]) {
    AqwertianFingeringAqwertian_ALGO_ = [AqwertianFingeringQwertyMapper_AlgorithmEnum USAGE];
  }
}
